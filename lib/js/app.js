(function($){
	var prototype = {
		...
		'pictureFrame' : function () {
			...
			function done(para) {
				$output.prepend("<div class='pf--done'><div class='pf__control'><button class='pf__edit'>edit</button><button class='pf__delete'>delete</button></div><p>" + para + "</p><small>" + moment().format('MMMM Do YYYY, h:mm:ss a') + "</small></div>");
				$input.val("");
				$input.trigger("focus");
			}
